...
def main():
    ...
    #load data
    train_dataset = CustomDataset('/home/jovyan/mnt/external-images-pvc/ximeng/csv_files_for_load/only_big_3_groups_train_dataset.csv', "/home/jovyan/scratch-shared/ximeng/croped_image"  )
    valid_dataset = CustomDataset('/home/jovyan/mnt/external-images-pvc/ximeng/csv_files_for_load/only_big_3_groups_test_dataset.csv', "/home/jovyan/scratch-shared/ximeng/croped_image"  )
    train_data_size = len(train_dataset)
    valid_data_size = len(valid_dataset)
    train_dataloader = torch.utils.data.DataLoader(train_dataset, batch_size=16, shuffle=True, num_workers=8)
    valid_dataloader = torch.utils.data.DataLoader(valid_dataset, batch_size=16, shuffle=True, num_workers=8)
    
    working_device = "cuda:1" #if torch.cuda.is_available() else "cpu"
    select_model,loss_function,optimizer = main_nn(working_device)

    num_epochs = 20
    file_save_name = '0312_groups_Resnet18_crop_20epoch'
